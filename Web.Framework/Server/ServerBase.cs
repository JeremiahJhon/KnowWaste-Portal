...
namespace Web.Framework.Server
{
    public class ServerBase : MSSQLServer
    {
        ...
        public ServerBase(string table)
        {
            Table = table;
            PageLimit = 10;
        ...
        //{
        //    Query = string.Format("SHOW COLUMNS FROM '{0}'", Table);
        ...
        public virtual int SelectCountQuery()
        {
            ...
            Query = string.Format("SELECT {4} {5}{0}{5}FROM {1}{2}", column, Table, Where.ToString(), Order.ToString(), Limit.ToString(), Environment.NewLine);
            Clear();
        ...
        {
            string query = string.Format("(SELECT {2} FROM {1} WHERE {0}.{1}_id={1}.id) AS {3}", Table, table, column, name);
        ...
        {
            Limit.Clear();
            //Limit.Append(string.Format(" LIMIT {0}", limit));
        ...
        {
            Query = string.Format("SELECT COUNT(*) FROM {1}{2}", 0, Table, Where.ToString(), Order.ToString(), Limit.ToString(), Environment.NewLine);
            Clear();
